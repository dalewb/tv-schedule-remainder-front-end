...
// If the element has any children, then move to the next element
// which would be the next day until there are no children, then
// incrementing the dayToMoveTo variable by 1 and cheching that
// element, until either dayToMoveTo reaches 7 or the element gets
// placed.
...
function renderSortedEpisode(episodes) {
  ...
  episodes.forEach(episode => {
    ...
    let p = `<p><strong>${episode.title}</strong></p>`
    // p.innerHTML = episode.title
